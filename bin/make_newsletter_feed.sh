...
curl --request GET --url "https://us14.api.mailchimp.com/3.0/campaigns?count=1000&list_id=50c3e6d6fe&fields=campaigns.archive_url,campaigns.settings.subject_line,campaigns.settings.title,campaigns.send_time&sort_field=send_time&sort_dir=DESC"  --user anystring:$MAILCHIMP_NEWSLETTER_KEY | jq '[.campaigns[] | {url: .archive_url, pub_date: .send_time, subject: .settings.subject_line, title: .settings.title} | select(((.title // "") | startswith("Resend") | not) and .pub_date != "")   ] | sort_by(.pub_date) | reverse' >   "$OUTPUT_PATH"/newsletter.json
