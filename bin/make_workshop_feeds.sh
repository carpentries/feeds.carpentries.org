...
## all
jq 'map(select(.end_date | strptime("%Y-%m-%d")? | mktime < now)) | sort_by(.start_date | strptime("%Y-%m-%d")? | mktime | reverse)' < "$OUTPUT_PATH"/all_workshops_plain.json > "$OUTPUT_PATH"/all_past_workshops_plain.json
...
## all
jq 'map(select(.end_date | strptime("%Y-%m-%d")? | mktime >= now)) | sort_by(.start_date | strptime("%Y-%m-%d")? | mktime)' < "$OUTPUT_PATH"/all_workshops_plain.json > "$OUTPUT_PATH"/all_upcoming_workshops_plain.json
